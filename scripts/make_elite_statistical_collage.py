...
OUT_FIG = os.path.join(PROJ, "outputs", "figures", "explainable_heat_interactions.png")

print("Loading data for statistical collage...")
df_ind = pd.read_csv(IND_PATH)

# Re-run XGBoost and SHAP to get exact cell-level SHAP values
...
# Create 1x2 panel layout (Panels a & b only)
fig, axes = plt.subplots(1, 2, figsize=(15.5, 6.5), facecolor="#ffffff")

# Smooth trendline drawer helper
...
# Panel a: SHAP Bivariate dependency for bld_cov, colored by f_green (Built vs. Green interaction)
ax0 = axes[0]
...
ax0.set_title("(a) Built-Green Interaction Effect on Heat", fontsize=14, fontweight="bold", color="#1e293b")
...
# Panel b: SHAP Bivariate dependency for dist_coast_km, colored by far (Coast vs. Density interaction)
ax1 = axes[1]
...
ax1.set_title("(b) Coastal Gradient & Density Joint Effect", fontsize=14, fontweight="bold", color="#1e293b")
