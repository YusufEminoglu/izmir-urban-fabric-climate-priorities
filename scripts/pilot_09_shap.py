...
print(f"spatial-block CV  morphology-only: R2={r2_m:.3f} RMSE={rmse_m:.2f}")
print(f"spatial-block CV  +coastal/topographic context: R2={r2:.3f} RMSE={rmse:.2f} degC")

# regularised linear baselines (same spatial-block CV) — guard against over-reading
# collinear predictors and isolate the non-linear gain of the gradient-boosting model
...
def cv_lin(cols, est):
    ...
    oof = np.zeros(len(y))
    for tr, te in gkf.split(Xx, y, groups=blocks):
        sc = StandardScaler().fit(Xx.iloc[tr])
        m = clone(est).fit(sc.transform(Xx.iloc[tr]), y[tr])
        oof[te] = m.predict(sc.transform(Xx.iloc[te]))
    return r2_score(y, oof), np.sqrt(mean_squared_error(y, oof))

r2_ridge_m, rmse_ridge_m = cv_lin(MORPH, Ridge(alpha=1.0))
r2_ridge_f, rmse_ridge_f = cv_lin(FEATURES, Ridge(alpha=1.0))
r2_en_m, rmse_en_m = cv_lin(MORPH, ElasticNet(alpha=0.1, l1_ratio=0.5, max_iter=10000))
r2_en_f, rmse_en_f = cv_lin(FEATURES, ElasticNet(alpha=0.1, l1_ratio=0.5, max_iter=10000))
print(f"linear baselines (spatial-block CV):")
print(f"  Ridge       morph R2={r2_ridge_m:.3f} RMSE={rmse_ridge_m:.2f} | full R2={r2_ridge_f:.3f} RMSE={rmse_ridge_f:.2f}")
print(f"  ElasticNet  morph R2={r2_en_m:.3f} RMSE={rmse_en_m:.2f} | full R2={r2_en_f:.3f} RMSE={rmse_en_f:.2f}")
X = prep(FEATURES)
...
imp.to_csv(os.path.join(TABDIR, "shap_global_importance.csv"), index=False)

# per-stratum dominant mechanism (mean signed SHAP per feature within stratum)
svdf = pd.DataFrame(sv, columns=FEATURES)
svdf["stratum_name"] = df["stratum_name"].values
per = svdf.groupby("stratum_name").mean()
per.to_csv(os.path.join(TABDIR, "shap_per_stratum_mechanism.csv"))
dom = per[MORPH].abs().idxmax(axis=1)

# Standardized strata ordering and formatting
strata_order = [
    "historic_core",
    "apartment_block",
...
# Re-name columns of X for summary plot
...
# global synthesis plot (2 panels)
...
# Right panel: Heatmap of per-stratum signed SHAP (strata as rows, features as columns)
ax1 = axes[1]
...
ax1.set_title("(b) Directional heat mechanisms by fabric", fontsize=14, fontweight="bold", color="#1e293b")
...
# Add a text box for model cross-validation diagnostics
metrics_text = (
...
out_path = os.path.join(FIGDIR, "shap_synthesis.png")
fig.savefig(out_path, dpi=300, facecolor=fig.get_facecolor(), edgecolor="none")
plt.close(fig)
print(f"wrote {out_path}")

print("\n=== global SHAP importance (top) ===")
print(imp.head(8).to_string(index=False))
print("\n=== dominant heat mechanism per stratum (signed mean SHAP, degC) ===")
for s in strata_order:
    if s in per.index:
        f = dom[s]
        print(f"  {s:26s} -> {f:16s} ({per.loc[s, f]:+.2f} degC)")
